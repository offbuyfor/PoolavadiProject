...
MODEL_PATH = os.environ.get("MODEL_PATH", "models/best_model_put.joblib")
LABEL_ENCODER_PATH = os.environ.get("LABEL_ENCODER_PATH", "models/label_encoder_put.joblib")
PREDICTION_FLAG_TYPE = "put"
POSITION_VALUE_COL = "total_investment"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch prediction runner (put model)")
...
def add_position_value(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    required = ["Stock_Price", "Put_Option_Price"]
    ...
    out[POSITION_VALUE_COL] = (pd.to_numeric(out["Stock_Price"], errors="coerce") * 100) + (
        pd.to_numeric(out["Put_Option_Price"], errors="coerce") * 100
    )
    return out
